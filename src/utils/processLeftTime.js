...
export default function processLeftTime (store) {
	...
	let tick = () => {
		...
		if (!router.app.$route.path.includes('login')) {
			let token = getToken();
			let time = getToken('expireAt');
			if ((/\d+/g).test(time)) {
				time *= 1;
			}
			if (token) {
				store.commit('SET_LEFTTIME', new Date(time) - Date.now());
			} else {
		...
		/*
			...
			选择：
				...
				3.重新登录后再次启动
		*/
		if (cacheTime != 0 && store.getters.leftTime == 0 && !router.app.$route.path.includes('login')) {
